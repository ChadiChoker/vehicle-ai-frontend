...
export default function App() {
  ...
  const createInspection = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/inspections`, { method: "POST" });
  ...
  const uploadPhoto = async (file: File | null, type: "pickup" | "return") => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/inspections/${inspectionId}/photos`, {
        method: "POST",
        body: formData,
      });
  ...
  const analyzeInspection = async () => {
    ...
    setLoading(true);
    try {
      const analyzeRes = await fetch(`${API_BASE_URL}/api/inspections/${inspectionId}/analyze`, {
        method: "POST",
      });
      if (!analyzeRes.ok) throw new Error("Analyze failed");

      const resultsRes = await fetch(`${API_BASE_URL}/api/inspections/${inspectionId}/results`);
